...
# Remark: I'm not entirely sure when extensive use of logging is recommended, but 
# to me it seems a bit excessive here. Logging is useful when you don't have
# access to the program while it's running (e.g., if it's running on a server).
# Here, unit tests and debugging should give you enough access such that you don't 
# need that much logging.
# Don't get me wrong, I think it's great that you're trying to make your life eaiser
# and I do see the value in logging, but to me the downsides (code is less readable 
# and the logging requires a lot of maintenance as well) outweigh the upsides here.

# Remark: Overall, nice work here! I'm happy to see that you've build a complete
# game suite yourself, and the user interface is very nice. Your coding style is 
# quite clear and readable. It's a bit more imperative than most Python code,
# but that's probably due to your background and not a problem at all.
# There are a few spots where it's not completely clear which part of the code does 
# what (partly a naming issue, partly because functionality is 'spread' across several
# methods). Maybe it would be worth trying to split up the class a bit more, e.g.,
# separate the game logic from the user interface logic, have a board class, or 
# write a class that runs the game.
...
class Connect4:
    ...
    # Remark: Methods also need to be documented.
    """
    ...
    def get_valid_moves(self) -> list[tuple[int, int]]:
        ...
        logger.debug("called")
        valid_moves = []
        for col in range(self.col):  # Remark: Just checking whether the highest row is empty would be enough.
    ...
    def check_winner(self, player: str) -> bool:
        ...
        """
        logger.debug("called")
        return check_winner(board=self.board, player=player, win=self.win)  # Remark: You're using the same name twice here.
    ...
    def make_move(self, player: str, col: int) -> bool:
        ...
        """
        # Remark: It's a bit confusing to have methods 'move' and 'make_move'.
        # The difference between them are the checks, which happen in two separate methods though.
        # I think it would be better to either completely separate the checks from the move and
        # always call them in succession, or have one method that does the checks, and one method
        # called 'check_and_move' that first calls the method with the checks and then makes the move.
        logger.debug("called")
        move = self.check_move(col=col)
        if not isinstance(move, tuple): # Remark: Maybe make that part of the check_move function?
            return False
    ...
    def play_game(self, type_: str, first_move: str = "") -> None:
        ...
        """
        # Remark: This function should probably be split into smaller functions.
        # E.g., the parts that require player input could be their own functions.
        logger.debug("called")
        self.init_board()
        turn = 0
        player = Players.P1
